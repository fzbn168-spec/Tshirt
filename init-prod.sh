...
# 2. 等待服务就绪
echo -e "\n${GREEN}[2/4] 等待服务启动 (10秒)...${NC}"
sleep 10

# 3. 运行数据库迁移 (确保表结构存在)
echo -e "\n${GREEN}[3/4] 正在运行数据库迁移...${NC}"
docker compose -f docker-compose.prod.yml exec -T backend npx prisma migrate deploy

# 4. 运行数据填充
echo -e "\n${GREEN}[4/4] 正在填充初始数据...${NC}"
...
echo -e "\n${GREEN}=== 初始化完成 ===${NC}"
echo "接下来请创建管理员账号："
echo "docker compose -f docker-compose.prod.yml exec backend node dist/scripts/create-admin.js"
