...
export class ShippingsService {
  ...
  async create(createShippingDto: CreateShippingDto) {
    ...
    if (order.status === 'SHIPPED' || order.status === 'COMPLETED' || order.status === 'CANCELLED') {
        throw new BadRequestException(`Order cannot be shipped (Current status: ${order.status})`);
    }
    ...
    if (fullOrder?.user) {
        await this.notificationsService.notifyUser(
            ...
            EmailTemplates.shippingUpdate(fullOrder.user.fullName || 'Customer', fullOrder.orderNo, carrier, trackingNo),
            orderId,
            'ORDER'
        );
    }
